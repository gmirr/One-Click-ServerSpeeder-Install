...
echo ""
echo "#############################################################"
echo "# One click Install serverSpeeder(Debian/Ubuntu)            #"
echo "# WelCome to visit: https://iforday.com/39.html             #"
echo "# Happy new year!                                           #"
echo "# 2016/2/9                                                  #"
echo "#############################################################"
echo ""
...
#安装必要工具
        apt-get -y update
        apt-get -y install python python-dev python-pip curl wget unzip  make perl unzip vim macchanger
#生成文件		
        cat << _EOF_ >mac.py
S='${key}'
...
[ -n "eth0" ] && sed -i "s/^accif=.*/accif=\"eth0\"/" $ROOT_PATH/etc/config
[ -n "1000000" ] && {
